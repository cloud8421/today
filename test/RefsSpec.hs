  ...
  describe "Tasks" $ do
    ...
    describe "resolving refs" $ do
      it "replaces with the corresponding entry" $ do
        let ref = Refs.Ref "T" "789" "T#789"
        let expectedText = "https://github.com/cloud8421/t/issues/789"
        Refs.resolveRef ref Refs.defaultRefMap `shouldBe` expectedText
      it "returns an error text when unable to resolve" $ do
        let ref = Refs.Ref "OTHER" "789" "T#789"
    ...
    describe "replace refs" $
      it
        ...
        let text = "Fix issue T#456 and OTHER#789"
        let expectedText =
              "Fix issue https://github.com/cloud8421/t/issues/456 and OTHER#789"
